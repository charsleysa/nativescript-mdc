...
function initMDCStackLayout() {
    if (!MDCStackLayout) {
        if (isPostLollipopMR1()) {
            MDCStackLayout = org.nativescript.widgets.StackLayout;
        } else {
            initializePreLollipopStackLayout();
            MDCStackLayout = PreLollipopStackLayout as any;
        }
    }
}
...
export class Ripple extends RippleBase {
    ...
    public createNativeView() {
        initMDCStackLayout();
        const view = new MDCStackLayout(this._context);
